...
# ==============================================================================
# 📅 Last Sync:       2026-01-19 03:00 (GMT+8)
...
**主要任务：** 彻底修复 "Ingestion & Extraction" 阶段的工程鲁棒性问题，彻底修复 **Bug E: 实体命名不规范导致无法对齐** 。
...
5. 实体名称必须规范化，严禁出现括号（如 洁净室（区）），缩写必须强制大写（如 ahu -> AHU），同义词必须映射为同一名称（如 洁净区 -> 洁净室）。
# ==============================================================================
...
### 🔴 Bug E: 实体命名不规范导致无法对齐
*   **Location:** `core/utils.py`, `Prompt/*.txt`, `core/agents.py`
*   **Symptom:** 
    *   同一实体碎片化：`洁净室`, `洁净室（区）`, `洁净区` 成为 3 个无法链接的节点。
    *   括号干扰：`风机盘管（FP）`, `防火阀（FD）` 包含括号，导致匹配失败。
    *   缩写大小写不一：`AHU`, `ahu`, `AhU` 被识别为不同实体。
*   **Root Cause:** 
    1.  **Prompt 缺位**: Agent Prompt 中没有“命名规范”约束，模型输出随意。
    2.  **缺乏清洗**: `extract` 输出后，代码层没有对 `entities` 列表进行标准化清洗。
    3.  **括号陷阱**: LLM 习惯用括号合并同义词（如 `Item (Sub)`），但 Neo4j 需要精确匹配。
*   **Requirement:** 
    1.  **代码实现**: 在 `core/utils.py` 增加 `EntityNormalizer` 类。
        *   实现规则：去括号、强制大写缩写、同义词映射（如“洁净区”->“洁净室”）。
    2.  **Prompt 约束**: 修改所有 Agent Prompt（1, 2, 3, 4），在末尾增加“命名规范”章节（禁括号、强制大写）。
    3.  **集成 Agents**: 在 `agents.py` 的 `BaseAgent` 或 `extract` 方法中，解析 JSON 后先调用 `EntityNormalizer.normalize()`，再实例化 Entity 对象。
...
**任务目标：** 解决 Section 4 中列出的 **Bug E: 实体命名不规范导致无法对齐** 。

请按照以下顺序执行，**一次只解决一个问题**，待我确认后再进行下一个：

### 🏁 任务五：修复实体命名规范
1.  **分析**: 查看 Section 4 - Bug E。命名混乱导致图谱实体碎片化，无法链接。
2.  **执行**:
    *   **任务 5.1 (优先)**: 修改 `core/utils.py`。编写 `EntityNormalizer` 类，包含去括号、大小写转换和同义词映射逻辑。
    *   **任务 5.2 (Prompt)**: 修改 `Prompt/1 激进派.txt` 到 `4 大法官.txt`。统一的命名约束指令。
    *   **任务 5.3 (集成)**: 修改 `core/agents.py`。在 `extract` 方法返回前，调用 `EntityNormalizer` 进行清洗。
3.  **输出**: 提供修改后的 `utils.py` (Normalizer类), `agents.py` (BaseAgent集成), 以及任一 Prompt 示例。
