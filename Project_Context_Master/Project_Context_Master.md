### 📋 文件名：Project_Context_Master.md

```markdown
## 0. 头部元数据 (Header Metadata) 
# 🛡️ SYSTEM MANIFEST: ArchRAG (暖通智脑)
# ==============================================================================
# 📅 Last Sync:       2026-01-14 03:00 (GMT+8)
...
**主要任务：** 彻底修复 "Ingestion & Extraction" 阶段的工程鲁棒性问题，重点解决 **JSON 解析崩溃** 和 **Agent 输出截断**。
...
3.  大法官（Judge）的 Prompt 必须包含防止 Token 截断的策略（如分离推理与结果）。
# ==============================================================================
...
### 🔴 Bug B: 大法官输出截断 (Judge Output Truncation)
*   **Location:** `core/agents.py` -> `JudgeAgent.run()`
*   **Symptom:** 图谱生成流程卡死，或 JSON 数据缺失闭合括号 `}`。
*   **Root Cause:** `DeepSeek-Reasoner` (R1) 的思维链 (Reasoning Content) 过长，挤占了 Response 的 Token 空间，导致最终的 JSON 结果被截断 (Finish Reason: Length)。
*   **Requirement:** 
    1.  优化 Prompt，强制要求简短推理。
    2.  **关键：** 在 API 调用层面，如果可能，分离 `reasoning_content`，或者增加 `max_tokens` 上限。
    3.  增加“断点续传”逻辑：如果检测到 JSON 不完整，自动发起第二次请求补全。

### 🔴 Bug C: 实体与属性的 Schema 混淆 (Node Explosion)
*   **Location:** `neo4j_nodes.csv` (Output Artifact)
*   **Symptom:** 图谱中出现大量无意义的孤立节点，如 `3000m²`, `50Pa`, `≥5次/h`。
*   **Root Cause:** 激进派（Radical Agent）提取了这些作为实体，而大法官（Judge）未能将其降级为属性。这会导致图谱查询性能下降且语义混乱。
*   **Requirement:** 在 `core/models.py` 或 `Judge` 的 Prompt 中增加“数值清洗过滤器”。任何纯数值或单位数值，只能作为 Relationship 的 Property，严禁作为 Entity Node。
...
**任务目标：** 解决 Section 4 中列出的 **Bug A (JSON Crash)** 和 **Bug B (Judge Truncation)**。
...
### 🏁 任务二：优化大法官 Agent 的 Prompt (Priority: Medium)
*(完成任务一后，再执行此条)*
1.  **分析：** 查看 `Section 4 - Bug B`。`DeepSeek-Reasoner` 的思维链太长导致截断。
2.  **执行：** 修改 `Prompt/4 大法官.txt`。
3.  **要求：**
    *   在 System Prompt 中明确：“你的思考过程请放在 `<thinking>` 标签内，但最终 JSON 必须在代码块中。”
    *   或者，指示模型：“请尽可能简短地进行推理”。
